...
Worker::Worker(void)
{
    worker_thread = new asio::thread([this]() {
        ...
        //m_io_context_ptr = new asio::io_context();
...
void Worker::add_connection(asio::ip::tcp::socket *socket_ptr, RequestHandler &req_handler)
{
    m_connection_manager.start(std::make_shared<Connection>(std::move(*socket_ptr), m_connection_manager, req_handler));
    //m_connection_manager.start(std::make_shared<Connection>(socket_ptr, m_connection_manager, req_handler));
}
